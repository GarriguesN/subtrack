...
export default function EditExpensePage() {
  ...
  useEffect(() => {
    fetch(`/api/expenses?month=&category=`)
      .then(r => r.json())
      .then((data: any[]) => {
        const found = data.find((e: any) => e.id === parseInt(id));
        if (found) {
          setName(found.name);
          setAmount(String(found.amount));
          setPeriod(found.period || 'monthly');
  ...
  // Fetch logo when name is loaded
  useEffect(() => {
    if (!name.trim()) return;
    ...
    fetch(`/api/logo?name=${encodeURIComponent(name.trim())}`)
      ...
      .catch(() => {})
      .finally(() => setSearchingLogo(false));
  }, [name]);
  ...
  if (error && !name) {
    return (
      <div className="max-w-lg mx-auto text-center py-12">
        ...
        <button onClick={() => router.push('/')} className="btn btn-primary mt-4">Go back</button>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            <input type="text" value={name} onChange={e => setName(e.target.value)} className="input w-full" autoFocus style={{ paddingLeft: '2.5rem' }} />
            <div
              className="absolute left-3 top-1/2 -translate-y-1/2 cursor-pointer"
              onClick={() => allLogos.length > 0 && setShowLogoModal(true)}
              title={allLogos.length > 0 ? `Choose logo (${allLogos.length} available)` : ''}
            >
              {searchingLogo ? (
        ...
        <div>
          ...
          <div className="relative">
            <Euro size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)] pointer-events-none" />
        ...
        <div>
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} className="input min-h-[80px] resize-y" style={{ padding: '.625rem .75rem' }} />
        </div>
