...
export default function PinGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem('subtrack_unlocked')) {
      setUnlocked(true);
      return;
    }

    // Check if PIN is configured on the server
    fetch('/api/pin')
      .then(r => r.json())
      .then(data => {
  ...
  if (!pinConfigured && !settingPin) {
    return (
      <div className="min-h-dvh flex flex-col items-center justify-center p-6 bg-[var(--bg-primary)]">
        ...
        <h1 className="text-lg font-bold mb-2">Welcome to SubTrack</h1>
        <p className="text-sm text-[var(--text-secondary)] text-center mb-6">
          Set a PIN to protect your expenses
        </p>
        <button
  ...
  return (
    <div className="min-h-dvh flex flex-col items-center justify-center p-6 bg-[var(--bg-primary)]">
      ...
      <h1 className="text-lg font-bold mb-1">SubTrack</h1>
